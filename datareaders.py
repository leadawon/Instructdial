...
def get_reader(args, dataset):
    ...
    datasetconfig = config.get(dataset, None) #다시 *** config.json 파일에서 dataset에 해당하는 key를 찾는다.
    if datasetconfig is None and 'dataset_configs' in config and config['dataset_configs'].get(dataset,
                                                                                               None) is not None: # config.json에 dataset키가 없지만 config.json에 dataset_configs가 있고
                                                                                                                    # dataset_configs라는 key에 해당하는 value-> 에 dataset이름의 key가
                                                                                                                    # 있을때
        datasetconfig = config['dataset_configs'].get(dataset, None) # 그럼 "dataset_configs" -> dataset(이름) 에 해당하는 값을 datasetconfig에 저장할 것이다.
        ## TODO add logger
        # print('Found dataset config', datasetconfig)
        print('Found dataset config')
    if 'intent-clinc' in dataset: # dataset의 이름에서 있는지 확인한다.

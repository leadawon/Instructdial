...
from seq2seq_trainer import CustomSeq2SeqTrainer
...
# raw_datasets = load_dataset(extension, data_files=data_files, cache_dir=model_args.cache_dir)
    ...
    print('to check model.config.decoder_start_token_id', model.config.decoder_start_token_id)
    ...
    if training_args.do_train:
        column_names = train_dataset.column_names#raw_datasets["train"].column_names
    elif training_args.do_eval:
        column_names = eval_dataset.column_names#raw_datasets["validation"].column_names
    elif training_args.do_predict:
        column_names = test.column_names#raw_datasets["test"].column_names
    else:
    ...
    def preprocess_function(examples):
        ...
        for i, inp in enumerate(inputs):
            # if len(inp.split())>800:
            #     print(inp)
            #     inp = ' '.join(inp.split()[:800])
                # continue
